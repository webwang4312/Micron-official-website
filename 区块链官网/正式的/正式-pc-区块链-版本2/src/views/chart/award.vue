...
export default {
  ...
  methods: {
    async drawLine() {
      ...
      //  console.log(res.result.pastGasList);

      for (var i = 0; i < res.result.pastGasList.length + 1; i++) {
        avg_gas_for_7.unshift(res.result.pastGasList[i].amount);
        avg_gas_for_7time.unshift(
          this.timestampToTime2(res.result.pastGasList[i].createTime).substring(
            5
          )
        );
    ...
    async drawLine2() {
      let myChart2 = echarts.init(
        document.getElementById("average_blockreward")
      );
      ...
      for (var s = 0; s < res.result.pastBlockList.length + 1; s++) {
        avg_gas_for_72.unshift(res.result.pastBlockList[s].blockAward);
        avg_gas_for_7time2.unshift(
          this.timestampToTime2(
            res.result.pastBlockList[s].tradeTime
          ).substring(5)
        );
